function createAllDLBingReport() {//所有地类饼形图
    ...
    $.ajax({url:GEOSERVER.IP + GEOSERVER.PORT + '/getAllDltbAreaByProvinceCode', type: 'POST', data:{"proviceCode":getCountryCode(right)}, xhrFields:{withCredentials:true}, success:function(result){
        ...
        option = {
            ...
            series: [{name: right.name, type: 'pie', radius: '55%', center: ['80%', '50%'], data: eval(seriesData), emphasis: {itemStyle: {shadowBlur: 10, shadowOffsetX: 0, shadowColor: 'rgba(0, 0, 0, 0.5)'}}}]
        };
...
function createAllDLZhuReport() {//所有地类柱形图
    $.ajax({url:GEOSERVER.IP + GEOSERVER.PORT + '/getAllDltbAreaByProvinceCode', type: 'POST', data:{"proviceCode":getCountryCode(right)}, xhrFields:{withCredentials:true}, success:function(result){
        ...
        option = {title: {text:right.name +'所有地类面积报表',left: 'center'}, xAxis: {type: 'category', data: eval(xAxisData), axisLabel:{interval:0, rotate:45,}}, yAxis: {type: 'value'},
